...
export class CheckoutService {
  ...
  updateCheckout = async (c: Context) => {
    ...
    const idempotencyKey = c.req.header('Idempotency-Key');
    const ucpAgent = c.req.header('UCP-Agent');
    // Use c.req.valid('json') to get already-validated data from zValidator middleware
    // Don't use c.req.json() as body was already consumed by validation
    const updateRequest = c.req.valid('json') as ExtendedCheckoutUpdateRequest;
    let requestHash = '';
