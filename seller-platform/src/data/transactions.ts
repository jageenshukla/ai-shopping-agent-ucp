...
export function getAllOrders(): Order[] {
  const db = getTransactionsDb();
  const stmt = db.prepare('SELECT data FROM orders ORDER BY id DESC');
...
export function getAllCheckouts(): ExtendedCheckoutResponse[] {
  const db = getTransactionsDb();
  const stmt = db.prepare('SELECT data FROM checkouts ORDER BY id DESC');
  const results = stmt.all() as {data: string}[];

  return results.map(row => {
    try {
      return JSON.parse(row.data) as ExtendedCheckoutResponse;
    } catch (e) {
      console.error('Failed to parse checkout data', e);
      return null;
    }
  }).filter(Boolean) as ExtendedCheckoutResponse[];
}
