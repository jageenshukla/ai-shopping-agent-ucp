...
app.use(async (c: Context, next: () => Promise<void>) => {
  ...
  // Log UCP request-id header for request traceability
  const ucpRequestId = c.req.header('request-id');
  if (ucpRequestId) {
    c.env.incoming.log.info({ucpRequestId}, 'UCP request tracking');
  }
...
app.get('/admin', (c: Context) => {
  const html = readFileSync(join(__dirname, 'views', 'admin.html'), 'utf-8');
  return c.html(html);
});
...
/* Products endpoints (merchant-specific extension) */
app.get('/products', productsService.listProducts);
app.get('/products/:id', productsService.getProduct);
...
/* Admin endpoints */
app.get('/admin/orders', adminService.listOrders);
app.get('/admin/checkouts', adminService.listCheckouts);
app.get('/admin/inventory', adminService.getInventoryStatus);
